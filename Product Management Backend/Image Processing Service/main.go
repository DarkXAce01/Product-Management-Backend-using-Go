...
func updateCompressedImagesInDB(productID int, s3URL string) error {
	pgConnStr := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable", host, port, user, os.Getenv("DB_PASSWORD"), dbname)
...
func processQueueMessages(ch *amqp091.Channel, queue string, wg *sync.WaitGroup) {
	...
	for msg := range msgs {
		...
		s3URL, err := processImage(imageURL,  os.Getenv("S3_BUCKET"), imageQuality)
		if err != nil {
